...
class LearnHam:
    ...
    def trainmodel(self, savetodisk=True):
        ...
        def cg(theta, epsilon=5e-3):
            ...
            while(np.linalg.norm(rk)>epsilon):
                ...
                rk1 = rk - alphak*hessvec
                betak = np.matmul(np.transpose(rk1),rk1)/np.matmul(np.transpose(rk),rk)
                pk = rk1 + betak*pk
                rk = rk1
                print('rk',np.linalg.norm(rk))
            return theta
        self.theta = cg(theta)
    ...
    def graphcomparetraj(self, traj1, traj2, groundtruth, myfigsize=(8,16), includeField=False, fname='prop.pdf', mytitle=None):
        ...
        for item in [nzreals_keys[i:i + n] for i in range(0, len(self.nzreals), n)]:
            pls=pls+1
            ...
            for ij in item:
                ...
                thislabel = 'Re(P' + str(ijprime) + ')'
                mylabels.append(thislabel)
                ctr += 1
            plt.subplots_adjust(wspace=0, hspace=0,right=0.8,left=0.14)
            cnt = 0
            for ax in axs.flat:
            ...
            for ax in axs.flat:
                ax.label_outer()
            fig.savefig(self.outpath + mol+"Real"+str(pls)+ fname)
            plt.close()
        pls=0
        nzimags_keys=list(self.nzimags.keys())
        for item in [nzimags_keys[i:i + n] for i in range(0, len(self.nzimags), n)]:
            pls=pls+1
            fig = plt.figure(figsize=(myfigsize))
            mylabels = []
            axs = fig.subplots(n)
            ...
            fig.savefig(self.outpath + mol+"Imag"+str(pls) + fname)
            plt.close()
        return True
